...
If you don't have it already, you can [get `rustup`](https://www.rust-lang.org/install.html) from the appropriate page on our website, and check out the [detailed release notes for 1.78.0](https://doc.rust-lang.org/nightly/releases.html).

If you'd like to help us out by testing future releases, you might consider updating locally to use the beta channel (`rustup default beta`) or the nightly channel (`rustup default nightly`). Please [report](https://github.com/rust-lang/rust/issues/new/choose) any bugs you might come across!

## [#what-s-in-1-78-0-stable](#what-s-in-1-78-0-stable) What's in 1.78.0 stable

### [#diagnostic-attributes](#diagnostic-attributes) Diagnostic attributes
...
For more information, see the reference section on [the `diagnostic` tool attribute namespace](https://doc.rust-lang.org/nightly/reference/attributes/diagnostics.html).

### [#asserting-unsafe-preconditions](#asserting-unsafe-preconditions) Asserting `unsafe` preconditions
...
### [#deterministic-realignment](#deterministic-realignment) Deterministic realignment
...
- [`pointer::align_offset`](https://doc.rust-lang.org/std/primitive.pointer.html) computes the offset needed to change a pointer to the given alignment. It returns `usize::MAX` if that is not possible, but it was previously permitted to *always* return `usize::MAX`, and now that behavior is removed.
- [`slice::align_to`](https://doc.rust-lang.org/std/primitive.slice.html) and [`slice::align_to_mut`](https://doc.rust-lang.org/std/primitive.slice.html) both transmute slices to an aligned middle slice and the remaining unaligned head and tail slices. These methods now promise to return the largest possible middle part, rather than allowing the implementation to return something less optimal like returning everything as the head slice.

### [#stabilized-apis](#stabilized-apis) Stabilized APIs

- [`impl Read for &amp;Stdin`](https://doc.rust-lang.org/stable/std/io/struct.Stdin.html)
...
- [`impl From&lt;TryReserveError&gt; for io::Error`](https://doc.rust-lang.org/stable/std/io/struct.Error.html)

These APIs are now stable in const contexts:

- [`Barrier::new()`](https://doc.rust-lang.org/stable/std/sync/struct.Barrier.html)

### [#compatibility-notes](#compatibility-notes) Compatibility notes
...
Check out everything that changed in [Rust](https://github.com/rust-lang/rust/releases/tag/1.78.0), [Cargo](https://doc.rust-lang.org/nightly/cargo/CHANGELOG.html), and [Clippy](https://github.com/rust-lang/rust-clippy/blob/master/CHANGELOG.md).

## [#contributors-to-1-78-0](#contributors-to-1-78-0) Contributors to 1.78.0
