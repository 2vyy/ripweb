...
mod domain_trust_tests {
    ...
    fn high_trust_domain_gets_positive_delta() {
        ...
        assert!(
            c.delta > 0.0,
            "high trust must give positive delta, got {}",
            c.delta
        );
    ...
    fn medium_trust_domain_gets_smaller_positive_delta_than_high() {
        ...
        let input_high = ScorerInput {
            result: &high_result,
            query: "tokio",
            engine_rank: 0,
        };
        let input_med = ScorerInput {
            result: &med_result,
            query: "tokio",
            engine_rank: 0,
        };
    ...
    fn low_trust_domain_gets_negative_delta() {
        ...
        assert!(
            c.delta < 0.0,
            "low trust must give negative delta, got {}",
            c.delta
        );
    }
...
mod blocklist_tests {
    ...
    fn blocklist() -> BlocklistConfig {
        BlocklistConfig {
            domains: vec!["w3schools.com".to_owned(), "geeksforgeeks.org".to_owned()],
        }
    }
    ...
    fn blocklisted_domain_gets_heavy_penalty() {
        ...
        assert!(
            c.delta <= -3.0,
            "blocklist penalty must be at most -3.0, got {}",
            c.delta
        );
...
mod url_pattern_tests {
    ...
    fn docs_rs_url_gets_boost() {
        ...
        assert!(
            c.delta > 0.0,
            "docs.rs URLs must get a boost, got {}",
            c.delta
        );
    ...
    fn reference_path_gets_boost() {
        ...
        assert!(
            c.delta > 0.0,
            "/reference/ path must get a boost, got {}",
            c.delta
        );
    }
    ...
    fn medium_com_host_gets_penalty() {
        ...
        assert!(
            c.delta < 0.0,
            "medium.com must get a penalty, got {}",
            c.delta
        );
    }
    ...
    fn github_io_docs_host_gets_boost() {
        ...
        assert!(
            c.delta > 0.0,
            "github.io docs hosts must get a boost, got {}",
            c.delta
        );
    }
}
...
mod project_match_tests {
    ...
    fn project_name_in_title_gives_boost() {
        ...
        let inp = ScorerInput {
            result: &r,
            query: "tokio async runtime rust",
            engine_rank: 0,
        };
        let c = project_match::score(&inp);
        assert!(
            c.delta > 0.0,
            "project name in title must boost, got {}",
            c.delta
        );
    ...
    fn project_name_in_host_gives_boost() {
        ...
        assert!(
            c.delta > 0.0,
            "project name in host must boost, got {}",
            c.delta
        );
    }
    ...
    fn generic_query_with_no_project_token_gets_zero() {
        ...
        assert_eq!(
            c.delta, 0.0,
            "no project token must give 0.0, got {}",
            c.delta
        );
    }
    ...
    fn hyphenated_crate_name_is_detected_as_project_token() {
        ...
        let inp = ScorerInput {
            result: &r,
            query: "serde-json serialization",
            engine_rank: 0,
        };
        let c = project_match::score(&inp);
        assert!(
            c.delta > 0.0,
            "hyphenated crate name must be detected, got {}",
            c.delta
        );
    }
}
...
mod snippet_relevance_tests {
    ...
    fn all_query_terms_in_snippet_gives_max_coverage() {
        let r = make_result(
            "https://tokio.rs/",
            Some("Tokio is an async runtime for Rust"),
        );
        let inp = ScorerInput {
            result: &r,
            query: "tokio async runtime",
        ...
        assert!(
            c.delta > 0.0,
            "full coverage must give positive delta, got {}",
            c.delta
        );
    ...
    fn no_query_terms_in_snippet_gives_zero() {
        let r = make_result(
            "https://example.com/",
            Some("Completely unrelated content here"),
        );
        let inp = ScorerInput {
            result: &r,
            query: "tokio async runtime",
    ...
    fn partial_coverage_gives_intermediate_delta() {
        ...
        let r = make_result(
            "https://tokio.rs/",
            Some("Tokio and async programming in Rust"),
        );
        let inp = ScorerInput {
            result: &r,
            query: "tokio async runtime",
...
mod domain_diversity_tests {
    ...
    fn first_occurrence_gets_zero_penalty() {
        ...
        assert_eq!(
            c.delta, 0.0,
            "first occurrence must get 0.0, got {}",
            c.delta
        );
    ...
    fn second_occurrence_gets_penalty() {
        ...
        assert!(
            c.delta < 0.0,
            "second occurrence must get negative delta, got {}",
            c.delta
        );
    }
