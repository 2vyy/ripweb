...
#[test]
fn batch_allows_missing_positional() {
    let cli = Cli::try_parse_from(["ripweb", "--batch"]).unwrap();
    assert!(cli.batch);
    assert!(cli.query_or_url.is_none());
}

#[test]
fn wikidata_allows_missing_positional() {
    ...
    .unwrap();
    assert!(cli.wikidata.is_some());
    assert!(cli.query_or_url.is_none());
}

#[test]
fn site_requires_query_mode() {
    let result = Cli::try_parse_from(["ripweb", "--site", "example.com", "rust"]);
    assert!(result.is_err());
}

#[test]
fn as_of_parses_with_positional_input() {
    let cli =
        Cli::try_parse_from(["ripweb", "--as-of", "2024-01-01", "https://example.com"]).unwrap();
    assert_eq!(cli.as_of.as_deref(), Some("2024-01-01"));
}

#[test]
fn tables_flag_parses() {
    let cli = Cli::try_parse_from(["ripweb", "--tables", "https://example.com"]).unwrap();
    assert!(cli.tables);
}
...
fn verbosity_default() {
    let cli = Cli::try_parse_from(["ripweb", "example"]).unwrap();
    assert_eq!(cli.verbosity, ripweb::verbosity::Verbosity::Standard);
    assert_eq!(cli.format, ripweb::verbosity::OutputFormat::Md);
    assert_eq!(cli.verbose, 0);
}

#[test]
fn verbosity_flag_accepts_compact() {
    let cli = Cli::try_parse_from(["ripweb", "--verbosity", "compact", "example"]).unwrap();
    assert_eq!(cli.verbosity, ripweb::verbosity::Verbosity::Compact);
}

#[test]
fn format_flag_accepts_structured() {
    let cli = Cli::try_parse_from(["ripweb", "--format", "structured", "example"]).unwrap();
    assert_eq!(cli.format, ripweb::verbosity::OutputFormat::Structured);
}
