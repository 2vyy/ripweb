use ripweb::{
    ...
    search::platforms::{hackernews::HnContent, reddit::RedditContent},
    verbosity::{OutputFormat, Verbosity},
};
use url::Url;
...
fn compact_mode_generic_emits_delimiter_and_link() {
    ...
    let out = format_generic("some text", &url, Verbosity::Compact);
    assert!(
...
fn balanced_mode_generic_emits_delimiter_and_snippet() {
    ...
    let out = format_generic(&text, &url, Verbosity::Standard);
    assert!(
        out.starts_with("# --- [Source:"),
...
fn verbose_mode_generic_emits_full_content() {
    ...
    let out = format_generic(&text, &url, Verbosity::Full);
    assert!(
        out.starts_with("# --- [Source:"),
        "must start with source delimiter"
    );
...
fn source_delimiter_strips_tracking_params() {
    ...
    let out = format_generic("text", &url, Verbosity::Compact);
    assert!(
...
fn compact_mode_reddit_title_only() {
    ...
    let out = format_reddit(&content, Verbosity::Compact);
...
fn balanced_mode_reddit_top_two_comments() {
    ...
    let out = format_reddit(&content, Verbosity::Standard);
    assert!(out.contains("# Rust is great"));
...
fn verbose_mode_reddit_full_comment_tree() {
    ...
    let out = format_reddit(&content, Verbosity::Full);
...
fn compact_mode_hn_title_only() {
    ...
    let out = format_hn(&content, Verbosity::Compact);
...
fn balanced_mode_hn_top_five_comments() {
    ...
    let out = format_hn(&content, Verbosity::Standard);
    assert!(out.contains("# Show HN: Ripweb"));
...
fn verbose_mode_hn_full_comments() {
    ...
    let out = format_hn(&content, Verbosity::Full);
...
fn compact_mode_search_link_list() {
    ...
    let out = format_search_results(&items, Verbosity::Compact, OutputFormat::Md);
...
fn balanced_mode_search_link_plus_snippet() {
    ...
    let out = format_search_results(&items, Verbosity::Standard, OutputFormat::Md);
    assert!(out.contains("> Snippet 1"));
}

#[test]
fn full_mode_search_detailed_card() {
    let items = vec![ripweb::search::SearchResult {
        title: "Result 1".into(),
        url: "https://r1.com".into(),
        snippet: Some("Snippet 1".into()),
    }];
    let out = format_search_results(&items, Verbosity::Full, OutputFormat::Md);
...
fn compact_mode_github_issue_list_format() {
    let issue = ripweb::search::platforms::github::GithubIssue {
        number: 1,
        title: "Bug".into(),
        body: Some("Description".into()),
        labels: vec![],
        user: ripweb::search::platforms::github::GithubUser {
            login: "alice".into(),
        },
        html_url: "https://github.com/a/b/issues/1".into(),
    };
    let comments = vec![ripweb::search::platforms::github::GithubComment {
    ...
    let out =
        ripweb::search::platforms::github::format_issue(&issue, &comments, Verbosity::Compact);
...
fn balanced_mode_github_issue_op_only() {
    ...
    let out =
        ripweb::search::platforms::github::format_issue(&issue, &comments, Verbosity::Standard);
    assert!(out.contains("Description"));
...
fn verbose_mode_github_issue_with_comments() {
    ...
    let out = ripweb::search::platforms::github::format_issue(&issue, &comments, Verbosity::Full);
...
fn output_never_longer_than_input() {
    let fixtures: &[&[u8]] = &[
        include_bytes!("extraction/generic/article_clean.html"),
        include_bytes!("extraction/generic/bloated_generic.html"),
    ];
